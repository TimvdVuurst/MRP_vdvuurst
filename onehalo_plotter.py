...
def plot_distribution_gaussian_mod(func, param_dict,data,bins,distname = 'Mod Gaussian', filename = 'Mfit', show = False, loglambda = False, sanitycheck = False):
    ...
    Qval = get_Qvalue(param_dict, data, bins, integral_func = integral_func, sanitycheck = False)
...
def _get_data_and_plot(func, mass_bin, r_bin, loglambda, show = False,
    ...
    param_path = param_path_base + f'/M_{mass_bin[0]}-{mass_bin[1]}/r_{r_bin[0]:.2f}-{r_bin[1]:.2f}{loglambda_str}.json'
    ...
    plot_distribution_gaussian_mod(func, param_dict, data, rice_bins(data.size), show = show, loglambda = loglambda, filename = plot_path)

def plot_onehalo_fit(mass_bins = None, r_bins = None, show = False, loglambda = False, verbose = False):
    if not mass_bins:
    ...
    if np.size(mass_bins) == 2: # just one massbin
        if np.size(r_bins) == 2: # just one rbin
            failed = _get_data_and_plot(func, mass_bins, r_bins, loglambda, show)
            if failed:
                return
            ...
            for r_bin in iterable:
                failed = _get_data_and_plot(func, mass_bins, r_bin, loglambda, show)
                if failed:
        ...
        if np.size(r_bins) == 2:
            ...
            for mass_bin in iterable:
                failed = _get_data_and_plot(func, mass_bin, r_bins, loglambda, show)
                if failed:
                    continue
        else:
            iterable = tqdm(mass_bins) if verbose else mass_bins
            for mass_bin in iterable:
                for r_bin in r_bins:
                    failed = _get_data_and_plot(func, mass_bin, r_bin, loglambda, show)
                    if failed:
...
if __name__ == '__main__':
    ...
    if args.lower_mass and args.upper_mass:
        mass_bin = [args.lower_mass, args.upper_mass]
    
    else:
        mass_bin = None
    
    if args.lower_radius and args.upper_radius:
        r_bin = [args.lower_radius, args.upper_radius]
    else:
        r_bin = None
    
    plot_onehalo_fit(mass_bin, r_bin, loglambda = bool(args.loglambda),
